...
| `run_policy`, `identity_args`, `RUN_ID`, `workflow-bootstrap`, `workflow-state` | Translate in public pages; keep exact names only in advanced runtime/reference material. | "resume behavior", "workflow state", "active run" |
| `rp1 agent-tools emit`, `emit database`, `rp1.db`, `canonical events`, `canonical status`, `arcade_tracked` | Remove from Arcade user help; route runtime details to advanced/internal material. | "workflow progress reporting", "local activity history", "status", "tracked workflow" |
...
| `ToolResult`, `SQLite FTS5`, `bm25`, JSON envelopes | Keep only in advanced tool/API reference. | "search results", "work artifact search" |

...
const SCORE_LEVELS = [
  {
    min: 9, max: 15, label: "Workflow-ready",
  ...
  {
    min: 16, max: 24, label: "High rp1 ROI",
  ...
  {
    min: 25, max: 36, label: "Critical workflow debt",
...
function ScoreBar({ value, max, color }) {
  ...
  return (
    <div style={{ background: "var(--ra-surface-alt)", borderRadius: 4, height: 8, overflow: "hidden", marginTop: 6 }}>
      <div
        style={{
          width: `${pct}%`,
          height: "100%",
...
function generateInsight(answers, sectionScores) {
  ...
  // sectionScores sorted DESCENDING by pct, so sorted[0] is the highest-friction
  // (weakest-readiness) section — the user's biggest bottleneck.
...
function AIInsight({ answers, sectionScores }) {
  ...
  return (
    <div style={{ fontSize: 14, color: "var(--md-default-fg-color--light)", lineHeight: 1.75, margin: 0 }}>
      <p style={{ margin: "0 0 12px", borderLeft: "3px solid var(--ra-accent)", paddingLeft: 14 }}>
        {overview}
      </p>
      {recommendations.map((rec, i) => (
        <p key={i} style={{ margin: "0 0 8px", borderLeft: "3px solid var(--ra-border)", paddingLeft: 14, fontSize: 13 }}>
          {rec}
        </p>
      ))}
    </div>
...
export default function App() {
  ...
  const sectionScores = SECTIONS.map((sec) => {
    ...
    const actual = qs.reduce((acc, q) => acc + (answers[q.id] || 0), 0);
      ...
      <div style={{ width: "100%", maxWidth: 640 }}>
        ...
        {step === "intro" && (
          <div style={{ animation: "fadeUp 0.5s ease" }}>
            ...
            <h1 style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 32, color: "var(--md-default-fg-color)", lineHeight: 1.2, margin: "0 0 16px" }}>
              Is your codebase ready for production-grade AI workflows?
            </h1>
            <p style={{ fontSize: 15, color: "var(--md-default-fg-color--lighter)", lineHeight: 1.7, margin: "0 0 32px" }}>
              9 questions. 3 minutes. A personalised diagnosis of where AI friction is costing your team the most — and which rp1 commands address it directly.
            </p>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 32 }}>
              {SECTIONS.map((s) => (
                <div key={s.id} style={{ background: "var(--ra-surface)", border: "0.5px solid var(--ra-border)", borderRadius: 10, padding: "12px 14px" }}>
                  <div style={{ fontSize: 18, marginBottom: 6 }}>{s.icon}</div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: s.color, letterSpacing: "0.06em", textTransform: "uppercase" }}>{s.id}</div>
                </div>
              ))}
            </div>
            ...
            <button
              ...
              style={{
                width: "100%", padding: "16px 0", background: "linear-gradient(135deg, var(--ra-accent-dark), var(--ra-accent))",
                border: "none", borderRadius: 10, color: "var(--ra-cta-text)", fontFamily: "'Space Grotesk'",
        ...
        {/* -- QUESTIONS --------------------------------------------- */}
        {step === "questions" && q && (
          <div key={currentQ} style={{ animation: "fadeUp 0.3s ease" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, color: section?.color, fontWeight: 600 }}>{section?.icon} {q.section}</span>
              </div>
              <span style={{ fontSize: 12, color: "var(--md-default-fg-color--lightest)" }}>{currentQ + 1} / {totalQ}</span>
            </div>
            <div style={{ background: "var(--ra-surface-alt)", borderRadius: 4, height: 3, marginBottom: 32, overflow: "hidden" }}>
              <div style={{ width: `${progressPct}%`, height: "100%", background: section?.color, transition: "width 0.4s ease", boxShadow: `0 0 6px color-mix(in srgb, ${section?.color} 40%, transparent)` }} />
            </div>

            <h2 style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 22, color: "var(--md-default-fg-color)", lineHeight: 1.3, margin: "0 0 8px" }}>
              {q.question}
            </h2>
            <p style={{ fontSize: 13, color: "var(--md-default-fg-color--lighter)", marginBottom: 28, lineHeight: 1.5 }}>{q.subtext}</p>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {q.options.map((opt) => (
                ...
                >
                  <span style={{ fontSize: 14, fontWeight: 500, color: "var(--md-default-fg-color)" }}>{opt.label}</span>
                  <span style={{ fontSize: 11, color: "var(--md-default-fg-color--lighter)", flexShrink: 0, marginLeft: 12 }}>{opt.hint}</span>
                </button>
              ))}
            </div>
        ...
        {step === "results" && (
          <div style={{ animation: "fadeUp 0.5s ease" }}>
            <div style={{ background: "var(--ra-surface)", border: `1px solid color-mix(in srgb, ${level.color} 20%, transparent)`, borderRadius: 14, padding: 24, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 10, fontWeight: 700, color: level.color, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 6 }}>
                    Readiness level
                  </div>
                  <div style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 20, color: "var(--md-default-fg-color)", lineHeight: 1.2 }}>
                    {level.label}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 10, color: "var(--md-default-fg-color--lighter)", marginBottom: 4 }}>Score</div>
                  <div style={{ fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 28, color: level.color }}>
                    {score}<span style={{ fontSize: 14, color: "var(--md-default-fg-color--lighter)", fontWeight: 400 }}>/36</span>
                  </div>
                </div>
              </div>
              <p style={{ fontSize: 13.5, color: "var(--md-default-fg-color--light)", lineHeight: 1.65, margin: 0 }}>{level.summary}</p>
            </div>

            <div style={{ background: "var(--ra-surface)", border: "0.5px solid var(--ra-border)", borderRadius: 14, padding: 20, marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--md-default-fg-color--lighter)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 16 }}>Section breakdown</div>
              {sectionScores.map((s) => (
                <div key={s.id} style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                    <span style={{ color: s.color, fontWeight: 600 }}>{s.icon} {s.id}</span>
                    <span style={{ color: "var(--md-default-fg-color--lighter)" }}>{s.score}/{s.max}</span>
                  </div>
                  <ScoreBar value={s.score} max={s.max} color={s.color} />
                </div>
              ))}
            </div>

            <div style={{ background: "var(--ra-surface)", border: "0.5px solid var(--ra-border)", borderRadius: 14, padding: 20, marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--md-default-fg-color--lighter)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 14 }}>
                Personalised diagnosis
              </div>
              <AIInsight answers={answers} sectionScores={sectionScores} />
            </div>

            <div style={{ background: "var(--ra-surface)", border: "0.5px solid var(--ra-border)", borderRadius: 14, padding: 20, marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "var(--md-default-fg-color--lighter)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 14 }}>
                Recommended starting commands
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {level.commands.map((cmd) => (
                  <span key={cmd} style={{ background: "var(--ra-surface-alt)", border: "0.5px solid color-mix(in srgb, var(--ra-accent) 27%, transparent)", borderRadius: 6, padding: "5px 10px", fontSize: 12, color: "var(--ra-accent)", fontFamily: "monospace", fontWeight: 600 }}>
                    {cmd}
                  </span>
                ))}
              </div>
            ...
            <a
              ...
              style={{
                ...
                border: "none", borderRadius: 10, color: "var(--ra-cta-text)",
                fontFamily: "'Space Grotesk'", fontWeight: 700, fontSize: 15,
                cursor: "pointer", textAlign: "center", textDecoration: "none",
                boxShadow: "0 0 24px color-mix(in srgb, var(--ra-accent) 20%, transparent)", letterSpacing: "0.02em",
              }}
            >

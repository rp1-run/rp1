...
When `resumed` is `true`, scan the feature directory `{{$RP1_ROOT}}/work/features/{FEATURE_ID}/` for `.md` files that may not be registered under the resumed run. Report these as `unregistered_artifacts` in the output so the calling skill can register them.

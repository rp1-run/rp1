...
2. **Find next available suffix**:
   ...
   ```
   {{OUTPUT_DIR}}/{{REVIEW_ID}}*.md
   ```

3. **Calculate suffix**:
   - No existing files → `-001`
   - Existing files → increment highest suffix
   - Format: `-NNN` (zero-padded 3 digits)

4. **Final path**: `{{OUTPUT_DIR}}/{{REVIEW_ID}}-NNN.md`
...
```json
{"path": "{{OUTPUT_DIR}}/{{REVIEW_ID}}-{{NNN}}.md"}
```

...
- If the child returns no response at all (distinct from malformed output), check whether `{workRoot}/features/{FEATURE_ID}/design.md` exists on disk. If it exists, continue as if `status = "success"` -- do not record a failure. If it does not exist, send the same child agent (not a newly spawned one) a reminder of its remaining contract steps: design-decisions artifact (§8), artifact registration (§9), completion output (§12). Handle the follow-up response with the same branches in this list; if it is also silent, or returns `status = "error"` or malformed output, record a planning failure.

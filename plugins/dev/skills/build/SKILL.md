...
- If the child returns no response at all (distinct from malformed output), send the same child agent (not a newly spawned one) a reminder of its remaining contract steps: design-decisions artifact (§8), artifact registration (§9), completion output (§12). Note that `{workRoot}/features/{FEATURE_ID}/design.md` may already exist from the silent attempt -- the child should resume from it, not restart. Handle the follow-up response with the same branches in this list; if it is also silent, or returns `status = "error"` or malformed output, record a planning failure. The child's returned contract, not the presence of `design.md` on disk, gates advancement.

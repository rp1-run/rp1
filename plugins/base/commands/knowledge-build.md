...
Phase 2 (Parallel):    5 Analysis Agents → JSON outputs
Phase 3 (Sequential):  Command → Merge JSON → Write KB files
```
...
1. **Spawn 5 analysis agents in parallel** (CRITICAL: Use a SINGLE message with 5 Task tool calls):

   **Agent 1 - Index Builder**:

   ```
   Use Task tool with:
   subagent_type: rp1-base:kb-index-builder
   prompt: "MODE={{mode}}. Analyze files for index.md. Repository type: {{repo_type}}. Monorepo projects: {{monorepo_projects}}. Current project: {{current_project_path}}. Files to analyze (JSON): {{stringify(index_files)}}. {{if mode==INCREMENTAL}}File diffs (JSON): {{stringify(file_diffs_for_index_files)}}{{endif}}. Return JSON with project data."
   ```
...
3. **Handle partial failures**:
   ...
   **If 1 agent fails**:
   - Continue with remaining 4 successful agents
   - Generate placeholder content for failed section:
     * index.md failed → "# Error generating index - see logs, run full rebuild"
   ...
   - Write partial KB files (4 successful sections + 1 placeholder)
...
2. **Merge data into templates**:

   **index.md**:
   - Use index-builder JSON data
   - Fill template sections: project info, structure, entry points, quick start
...
Next steps:
- Use /rp1-base:knowledge-load to load KB for KB-aware agents
- Subsequent runs will use same parallel approach (10-15 min)
- Only validation-driven updates use faster sequential approach (1-3 min)
```
...
3. **Parallel spawning**: Spawn 5 agents in SINGLE message with multiple Task calls
4. **Error handling**: Provide clear error messages with troubleshooting steps if failures occur
5. **No user interaction**: Complete entire workflow autonomously
6. **Set expectations**: Inform user builds take 10-15 minutes (or instant if no changes)
...
Now spawning 5 parallel agents...
Spawning kb-index-builder...
Spawning kb-concept-extractor...
Spawning kb-pattern-extractor...
etc. (too verbose!)
```

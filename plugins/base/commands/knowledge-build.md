...
3. **Determine build strategy**:
   ...
   **CASE C: Incremental update** (state.json exists AND commit changed AND files changed in CODEBASE_ROOT):
   ...
   - **Check change set size** (prevent token limit issues):
     ...
     ```bash
     # Count lines changed, excluding generated/lock files
     EXCLUDE_REGEX="(package-lock\.json|yarn\.lock|bun\.lockb|pnpm-lock\.yaml|\.lock$|/vendor/|/node_modules/|/dist/|/build/|\.generated\.|\.min\.|\.bundle\.)"

     # Get line change stats, filter generated files, sum insertions+deletions
     lines_changed=$(git diff --stat {{old_commit}} {{new_commit}} -- $changed_files 2>/dev/null | \
   ...
   - **MESSAGE**:
     ...
     - If MODE=INCREMENTAL: "Changes detected since last build ({{old_commit}} → {{new_commit}}). Analyzing ~{{lines_changed}} lines in {{changed_file_count}} files (2-5 min)"
   - **Get detailed diffs for each changed file** (only if MODE=INCREMENTAL):

     ```bash
     # Only if incremental mode (≤2000 lines)

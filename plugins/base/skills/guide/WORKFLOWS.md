...
| Step | Skill | Input | Output |
|------|-------|-------|--------|
| 0 | `/pr-walkthrough [target]` | PR number, URL, or branch | Validated Code Tour JSON walkthrough under `.rp1/work/pr-walkthroughs/` |
...
- `/pr-walkthrough` is an optional orientation step. It gathers direct PR metadata, changed files, diffs, and commits, then registers a validated Code Tour JSON artifact with concepts, source fragments, concise relationship labels, and guided tour steps. Arcade opens valid walkthrough artifacts in the 3D Code Tour reader with Source mode for inspection and diagnostics; the workflow does not use existing `pr-review` artifacts or post comments.

...
import type { AgentSubState, Step } from "../../types/runs.js";
...
function makeSubState(
	overrides: Partial<AgentSubState> & { agentName: string },
): AgentSubState {
	return {
		step: "building",
		status: "running",
...
describe("computeTaskCounts", () => {
	test("returns counts when tasks are present", () => {
		const subStates: AgentSubState[] = [
			makeSubState({
				agentName: "task-builder",
				task: "T1",
				status: "completed",
			}),
			makeSubState({
			...
			makeSubState({ agentName: "task-builder", task: "T3", status: "failed" }),
		];
		const result = computeTaskCounts(subStates);
		expect(result.taskCount).toBe(3);
		expect(result.completedTaskCount).toBe(2);
	});

	test("returns null for both counts when no tasks (all task=null)", () => {
		const subStates: AgentSubState[] = [
			makeSubState({
				agentName: "task-builder",
				task: null,
			...
			makeSubState({
				agentName: "task-reviewer",
				task: null,
				status: "running",
			}),
		];
		const result = computeTaskCounts(subStates);
		expect(result.taskCount).toBeNull();
		expect(result.completedTaskCount).toBeNull();
	});

	test("counts completed and failed as done (BR-001)", () => {
		const subStates: AgentSubState[] = [
			makeSubState({
				agentName: "task-builder",
				task: "T1",
			...
			makeSubState({ agentName: "task-builder", task: "T2", status: "failed" }),
			makeSubState({
				agentName: "task-builder",
				task: "T3",
				status: "running",
	...
	test("deduplicates task IDs (multiple sub-states for same task)", () => {
		const subStates: AgentSubState[] = [
			makeSubState({
				agentName: "task-builder",
				task: "T1",
		...
		// T1 has a running sub-state (reviewer), but also a completed sub-state (builder)
		// The function counts a task as done if ANY sub-state with that task is completed/failed
		expect(result.completedTaskCount).toBe(2);
	});

	test("mixed: some sub-states with tasks, some without", () => {
		const subStates: AgentSubState[] = [
			makeSubState({
				agentName: "task-builder",
				task: "T1",
			...
			makeSubState({ agentName: "planner", task: null, status: "completed" }),
			makeSubState({
				agentName: "task-builder",
				task: "T2",
				status: "running",
...
describe("attachAgentSubStatesToSteps - task counts", () => {
	test("populates task counts on steps with agent sub-states that have tasks", () => {
		const steps: Step[] = [
			makeStep({
				id: "build",
				status: "running",
		...
		const result = attachAgentSubStatesToSteps(
			steps,
			skillRecords,
			agentRecords,
		);
	...
	test("keeps null task counts on steps with agent sub-states but no task IDs (BR-003)", () => {
		const steps: Step[] = [
			makeStep({
				id: "build",
				status: "running",
		...
		expect(result[0].agentSubStates).toBeDefined();
		expect(result[0].taskCount).toBeNull();
		expect(result[0].completedTaskCount).toBeNull();
	});

	test("keeps null task counts on steps without agent sub-states (AC-009)", () => {
		const steps: Step[] = [
			makeStep({ id: "requirements", status: "completed" }),
			makeStep({
				id: "build",
				status: "running",
				startedAt: "2026-03-01T00:00:00.000Z",
		...
		expect(result[1].completedTaskCount).toBe(1);
	});

	test("returns steps unchanged when no agent records", () => {
		const steps: Step[] = [makeStep({ id: "build", status: "running" })];
		const result = attachAgentSubStatesToSteps(steps, [], []);
		expect(result[0].taskCount).toBeNull();
		expect(result[0].completedTaskCount).toBeNull();
		expect(result[0].agentSubStates).toBeUndefined();
	});
});

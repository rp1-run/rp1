...
import { ZoomIn, ZoomOut, RotateCcw, Code, Expand, X } from "lucide-react";
...
const DiagramFullscreenContext = createContext<DiagramFullscreenContextValue | null>(null);

interface DiagramFullscreenProviderProps {
  children: ReactNode;
}

export function DiagramFullscreenProvider({ children }: DiagramFullscreenProviderProps) {
  ...
  return (
    <DiagramFullscreenContext.Provider value={{ isFullscreen, openFullscreen, closeFullscreen, updateCode }}>
      {children}
      {fullscreenModal}
...
export function useDiagramFullscreen(): DiagramFullscreenContextValue {
  ...
  if (!context) {
    throw new Error("useDiagramFullscreen must be used within a DiagramFullscreenProvider");
  }
  return context;
}

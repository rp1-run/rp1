...
export const TOOL_MODULES: ReadonlyMap<string, () => Promise<void>> = new Map([
	...
	[
		"blueprint-context",
		() => import("./blueprint-context/index.js").then(() => undefined),
	],
	[
		"build-task-plan",
...
Available Tools:
  ...
  blueprint-context Persist blueprint interview context safely (write, read, delete)
...
const blueprintContextCommand = agentToolsCommand
	.command("blueprint-context")
	.description("Persist blueprint interview context safely")
	.addHelpText(
		"after",
		`
Description:
  Persists a blueprint interview's EXTRA_CONTEXT sidecar with safe, atomic
  writes. The payload arrives over stdin (never through the shell), is wrapped
  in a versioned JSON envelope, and is written via temp-file + rename so a
  reader never sees a partial or shell-interpreted value. Stored at
  {work-root}/blueprint/context/{key}.json.

Subcommands:
  write    Write a context payload atomically (payload via stdin)
  read     Read and validate a context payload
  delete   Delete a context payload idempotently

Examples:
  printf '%s' "$EXTRA_CONTEXT" | rp1 agent-tools blueprint-context write --key my-prd --work-root /path/to/.rp1/work
  rp1 agent-tools blueprint-context read --key my-prd --work-root /path/to/.rp1/work
  rp1 agent-tools blueprint-context delete --key my-prd --work-root /path/to/.rp1/work
`,
	);

blueprintContextCommand
	.command("write")
	.description(
		"Write a blueprint context payload atomically (payload via stdin)",
	)
	.requiredOption("--key <key>", "Context key (the effective PRD name)")
	.requiredOption("--work-root <dir>", "Absolute path to the work root")
	.option("-f, --file <path>", "Read the payload from a file instead of stdin")
	.addHelpText(
		"after",
		`
Description:
  Reads an opaque context payload from stdin (or --file) and writes it to
  {work-root}/blueprint/context/{key}.json using a random owner-only temp file
  and an atomic rename. The key is validated as a safe single path segment
  (letters, numbers, hyphen, underscore), rejecting traversal, separators,
  whitespace, and shell metacharacters. Because the payload never passes through
  the shell and the rename is atomic, arbitrary multiline/quoted/command-like
  content is stored verbatim and no reader observes a partial write.

Options:
  --key <key>         Context key / effective PRD name (required, safe slug)
  --work-root <dir>   Work root directory (required, resolved to absolute)
  -f, --file <path>   Read payload from a file instead of stdin (optional)

Output:
  JSON ToolResult with:
  - key: The context key
  - path: Absolute path to the written sidecar
  - bytes: Size of the serialized envelope

Examples:
  printf '%s' "$EXTRA_CONTEXT" | rp1 agent-tools blueprint-context write \\
    --key my-prd --work-root /path/to/.rp1/work
`,
	)
	.action(
		async (options: {
			...
			const toolName = "blueprint-context";
			await ensureToolLoaded(toolName);
			...
			const { writeBlueprintContext } = await import(
				"./blueprint-context/index.js"
			);
			const result = await writeBlueprintContext({
				key: options.key,
				content: inputResult.right.content,
			...
			console.log(
				formatOutput({ success: true, tool: toolName, data: result.right }),
			);
			process.exit(0);
		},
	);
...
blueprintContextCommand
	.command("read")
	.description("Read and validate a blueprint context payload")
	.requiredOption("--key <key>", "Context key (the effective PRD name)")
	.requiredOption("--work-root <dir>", "Absolute path to the work root")
	.addHelpText(
		"after",
		`
Description:
  Reads and validates the context payload at
  {work-root}/blueprint/context/{key}.json. A missing sidecar returns
  found=false (not an error). A present-but-unparseable, wrong-version, or
  key-mismatched file returns valid=false so a truncated or foreign payload is
  never treated as authoritative resume context.

Options:
  --key <key>         Context key / effective PRD name (required, safe slug)
  --work-root <dir>   Work root directory (required)

Output:
  JSON ToolResult with:
  - found: true/false
  - valid: true/false (when found)
  - content: The stored payload (when found and valid)
  - error: Reason string (when found and invalid)
  - path: Absolute path to the sidecar location

Examples:
  rp1 agent-tools blueprint-context read --key my-prd --work-root /path/to/.rp1/work
`,
	)
	.action(async (options: { key: string; workRoot: string }): Promise<void> => {
		const toolName = "blueprint-context";
		await ensureToolLoaded(toolName);

		const { readBlueprintContext } = await import(
			"./blueprint-context/index.js"
		);
		const result = await readBlueprintContext(options.workRoot, options.key)();
...
blueprintContextCommand
	.command("delete")
	.description("Delete a blueprint context payload idempotently")
	.requiredOption("--key <key>", "Context key (the effective PRD name)")
	.requiredOption("--work-root <dir>", "Absolute path to the work root")
	.addHelpText(
		"after",
		`
Description:
  Deletes the context payload at {work-root}/blueprint/context/{key}.json.
  Idempotent: succeeds with deleted=false when no sidecar exists.
...
Output:
  JSON ToolResult with:
  - deleted: true if a sidecar was removed, false if none existed
  - path: Absolute path to the sidecar location
...
`,
	...
	.action(async (options: { key: string; workRoot: string }): Promise<void> => {
		...
		const { deleteBlueprintContext } = await import(
			"./blueprint-context/index.js"
		);
		const result = await deleteBlueprintContext(
			options.workRoot,
			options.key,
		)();

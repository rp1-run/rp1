...
import type { BundledAssets, BundledPlugin, AssetEntry } from "./reader.js";
...
export const extractPlugins = (
  ...
  TE.tryCatch(
    ...
    (e) =>
      installError("extract-plugins", `Failed to extract plugins: ${e}`),
  );
...
export const getWebUIDir = (
  ...
  pipe(
    ...
    TE.chain((cached) =>
      cached !== null
        ? TE.right(cached)
        : extractWebUI(assets, onProgress),
    ),
  );

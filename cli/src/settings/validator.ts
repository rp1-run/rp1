 ...
 * 4. Model IDs are advisory: unrecognized IDs produce a warning listing the
 *    known set but are applied as-is — the harness is the authority on which
 *    model names exist, and new vendor models must be usable without an rp1
 *    release.
	...
	for (const [platformKey, tierMap] of Object.entries(config.platforms)) {
		...
		for (const [tier, modelId] of Object.entries(tierMap)) {
			if (!validIds.includes(modelId)) {
				warnings.push(
					`Model '${modelId}' for ${platform} tier '${tier}' is not in the known set (${validIds.join(", ")}); applying anyway — the harness will reject the ID if it does not exist`,
				);
			}
